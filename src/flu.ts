...
export function filter<T, S extends T>(iter: AsyncIterable<T>, f: (v: T, index: number) => v is S): AsyncIterable<S>
export function filter<T>(iter: AsyncIterable<T>, f: (v: T, index: number) => unknown): AsyncIterable<T>
export async function* filter<T>(iter: AsyncIterable<T>, f: (v: T, index: number) => unknown): AsyncIterable<T> {
    let index = 0
    for await (const i of iter) {
