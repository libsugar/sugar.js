...
export namespace Maybe {
     ...
     * ## Example
     * ```ts
     * let a: 2 = mapOr(1, 3, v => v + 1)
     * let b: 3 = mapOr(null, 3, v => v + 1)
     * ```
     */
    ...
    export function take<T>(v: Box<Maybe<T>>): Voidable<T> {
        if (isNone(v.value)) return
        return also(v.value, () => (v.value = void 0))
    }

    /** Replace the value in the box */
    export function replace<T>(v: Box<Maybe<T>>, val: T): Maybe<T> {
        return also(v.value, () => (v.value = val))
    }
